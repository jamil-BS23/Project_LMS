...
import { Upload, User2, CheckCircle2, Search, AlertCircle } from "lucide-react";
...
export default function UserSettings() {
  ...
  const onSave = async () => {
    try {
      ...
      if (res.status === 200 || res.status === 201) {
        // Success toast
        setToast({ show: true, msg: "User created successfully!", success: true });
        setSavePopup(true);
      } else {
        setToast({ show: true, msg: res.data.detail || "Failed to create user", success: false });
      }
    } catch (err) {
    ...
    // Hide toast after 2s
    setTimeout(() => setToast({ show: false, msg: "", success: true }), 2000);
  };
  ...
  return (
    <div className="min-h-screen flex bg-gray-100">
      ...
      {/* Toast notification */}
      {toast.show && (
        <div className={`fixed bottom-5 right-5 z-50 px-4 py-2 rounded shadow text-white flex items-center gap-2 ${toast.success ? "bg-green-500" : "bg-red-500"}`}>
