...
import api from "../../config/api"; // <-- preconfigured Axios instance
...
export default function AdminSettings() {
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (res.data) {
        setLimits({
          "borrow-day-limit": res.data.borrow_day_limit,
          "borrow-extend-limit": res.data.borrow_day_extension_limit,
          "borrow-limit": res.data.borrow_max_limit,
          "booking-duration": res.data.booking_duration ?? defaults["booking-duration"],
          "booking-days-limit": res.data.booking_days_limit ?? defaults["booking-days-limit"],
        });
      }
    } catch (err) {
      console.error("Error fetching settings:", err);
      setToast({ show: true, msg: "Failed to load settings", type: "error" });
  ...
  const doSave = async () => {
    ...
    try {
      ...
      setToast({ show: true, msg: "Settings saved successfully", type: "success" });
    } catch (err) {
      console.error("Error saving settings:", err);
      setToast({ show: true, msg: "Failed to save settings", type: "error" });
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen flex bg-gray-100">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`inline-flex items-center gap-2 rounded-md px-4 py-2 text-sm font-semibold text-white shadow ${
                loading ? "bg-gray-400" : "bg-sky-600 hover:bg-sky-500"
      ...
      {toast.show && (
        <div className="fixed bottom-6 right-6 z-[60] pointer-events-none animate-[toastIn_.25s_ease-out]">
          <div
            className={`pointer-events-auto flex items-start gap-3 rounded-xl shadow-lg ring-1 ring-black/5 px-4 py-3 ${
              toast.type === "success" ? "bg-white" : "bg-amber-100"
            }`}
          >
            <div className="mt-0.5">
              <CheckCircle2
                className={toast.type === "success" ? "text-green-600" : "text-amber-600"}
                size={22}
              />
            </div>
            <div>
