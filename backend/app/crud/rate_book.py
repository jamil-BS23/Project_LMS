...
class RateBookCRUD:
    ...
    @staticmethod
    async def rate_book(
        db: AsyncSession, book_id: int, rating: float, user_id: str
    ) -> Optional[Book]:
    ...
    async def get_rating_breakdown(db: AsyncSession, book_id: int) -> Dict:
        total_result = await db.execute(
            select(func.count(UserRating.rating_id)).where(
        ...
        for star in range(1, 6):
            count_result = await db.execute(
                select(func.count(UserRating.rating_id)).where(
                    UserRating.book_id == book_id, UserRating.rating == star
                )
            )
        ...
        for r in reviews:
            ...
            enriched_reviews.append(
                BookReviewOut.model_validate(r, from_attributes=True).model_dump()
            )
        return {
            "book_id": book_id,
            "total": total,
