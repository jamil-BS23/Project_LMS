...
export default function Sidebar({ onSelect }) {
  ...
  return (
    <aside className="hidden md:block w-64 bg-white p-4 border-r border-gray-200 sticky top-28 overflow-y-hidden space-y-6">
      ...
      {/* Course categories */}
      <div>
        <h3 className="text-base font-semibold mb-2 text-gray-900">Categories</h3>
                    ...
                    <label className="flex items-center gap-2 flex-1 cursor-pointer">
                      <input
                        ...
                        // onChange={() => toggleCategoryCheckbox(cat.name)}
                         onChange={() => {
                          if (activeCategory === cat.category_title) {
                            setActiveCategory(null);
                            if (onSelect) {
                            ...
                            setActiveCategory(cat.category_title);
                            if (onSelect) {
                              onSelect({ type: "category", value: cat.category_title });
                            } else {
                              navigate("/all-genres", {
                                state: { filter: { type: "category", value: cat.category_title } },
                              });
                            }
                          }
                        }}
